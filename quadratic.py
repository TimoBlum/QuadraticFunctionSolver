...
def drawPoint(x, y, width=1):
    pygame.draw.circle(win, (50, 255, 40), (x, y), width)


def drawLinear(m, point, color=(0, 0, 0)):
    pygame.draw.line(win, color, (point[0] - 200, point[1] - m * 200), (point[0] + 200, point[1] + m * 200))


def startingWin():
    global started
    if clickedOn((xy//2, xy//2), 150):
        started = True
    startImg = pygame.transform.scale(pygame.image.load("math background.jpg"), (xy, xy))
    win.blit(startImg, (0, 0, 0, 0))
    pygame.draw.circle(win, (0, 255, 0), (xy//2, xy//2), 150)
    textOnScreen(xy//2, xy//2, 'Click to get started!', '', 30, color=(255, 0, 0))


def showLocalTangent():
...
def func(a, b, c):
    ...
    while x < 600:
        ...
        localx = pygame.mouse.get_pos()[0]
        localy = xy/2 - (0.01*a*(((localx-xy/2)+b*100)**2)+c*100)
        drawPoint(localx, localy, width=10)
        m = (2*a*(xy/2 - localx) / 100) + (2*a*-b)
        drawLinear(m, (localx, localy))
